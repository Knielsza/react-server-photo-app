...
function App() {
  ...
  const [ipValue, setIp] = useState('192.168.1.248');
  const [selects, setSelects] = useState([]);

  const getPhotos = async() => {

      const response = await fetch('http://' + '192.168.1.248' + ':3001/getFiles', { method: "GET"})
  ...
  const delOne = async(id) => {
    if (!ipValue.includes('x')){
      await fetch('http://' + '192.168.1.248' + ':3001/deleteSth', {
        method: 'POST',
        headers: {
